...
function apbdop_edit_form(){
		...
		foreach ($results as $data) {
				...
				*/
				...
				if(user_has_role('3', $user)) {				//administrator
					...
					$akses_id = '6';
				} else if(user_has_role('7', $user)) {		//seksi
					$akses_id = '7';
				} else if(user_has_role('8', $user)) {		//auditor
					$akses_id = '8';
				}
	...
	if ($disabled) {
		...
		$form['username']['#default_value'] = $username;
	}
	$form['upwd']= array(
		'#type'         => 'textfield', 
		'#title'        => t('Password'), 
...
function apbdop_edit_form_validate($form, &$form_state) {
	...
	drupal_set_message($akses_id);
	drupal_set_message($kodesuk);
	drupal_set_message(strlen($kodesuk));
	
	if ($akses_id=='6') {
		if (strlen($kodesuk)!=4) {
			form_set_error("akses",'Hak akses tidak sesuai dengan Bidang/Seksi');
		}
	} elseif ($akses_id=='7') {
		if (strlen($kodesuk)!=6) {
			form_set_error("akses",'Hak akses tidak sesuai dengan Bidang/Seksi');

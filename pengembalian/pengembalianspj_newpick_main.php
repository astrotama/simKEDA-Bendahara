<?php
function pengembalianspj_newpick_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 20;
	...
	foreach ($results as $data) {
		...
		if ($cair>0) {
			...
			$editlink = apbd_button_baru_custom_small('pengembalianspj/barupost/' . $data->kodekeg . '/' . $jenis, 'SPJ');
	...
	//$output_form = drupal_get_form('pengembalianspj_newpick_main_form');
...
function pengembalianspj_newpick_main_form_submit($form, &$form_state) {

	
}


function pengembalianspj_newpick_main_form($form, &$form_state) {

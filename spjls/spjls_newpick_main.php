<?php
function spjls_newpick_main($arg=NULL, $nama=NULL) {
	$qlike='';
	$limit = 20;
	...
	if ($arg) {
		...
		//isset($_SESSION["spjls_newpick_bulan"])?$bulan = $_SESSION["spjls_newpick_bulan"]:$bulan = '1';
	...
	$query->join('kegiatanskpd', 'k', 'k.kodekeg=d.kodekeg');
	# get the desired fields from the database
	$query->fields('d', array('dokid', 'bulan', 'keperluan', 'kodeuk', 'spmno', 'sppno', 'sp2dno', 'sp2dtgl', 'jumlah', 'potongan', 'netto', 'spjsudah', 'spjsudah'));
	...
	if (isUserPembantu()) {
		...
		$query->condition('k.kodesuk', apbd_getusersuk(), '=');
	}
	...
	foreach ($results as $data) {
		...
		$rows[] = array(
					...
					array('data' => apbd_fd($data->sp2dtgl),'width' => '50px', 'align' => 'center', 'valign'=>'top'),
	...
	$output_form = drupal_get_form('spjls_newpick_main_form');
	return drupal_render($output_form) . $table;
	
}


function spjls_newpick_main_form_submit($form, &$form_state) {
	...
	*/
	...
	$_SESSION["spjls_newpick_bulan"] = $bulan;
	$_SESSION["spjls_newpick_spjsudah"] = $spjsudah;

	$uri = 'spjls/baru/filter/' . $bulan . '/' . $spjsudah;
	drupal_goto($uri);
	
}


function spjls_newpick_main_form($form, &$form_state) {
	$kodeuk = apbd_getuseruk();
	if(arg(2)!=null){

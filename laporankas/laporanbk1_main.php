...
function laporanbk1_main($arg=NULL, $nama=NULL) {
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {	
		...
		$output = getlaporanbk1_tmpl($kodeuk, $jenis, $tglawal, $tglakhir, $detil, $tglcetak);
		$output_form = drupal_get_form('laporanbk1_main_form');
...
function laporanbk1_main_form($form, &$form_state) {
...
function laporanbk1_main_form_validate($form, &$form_state) {
...
function laporanbk1_main_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['submit']) 
		$uri = 'laporanbk1/' . $kodeuk . '/' . $jenis . '/'  . $tglawal . '/' . $tglakhir . '/'. $detil . '/pdf/' . $tglcetak . '/' . $marginatas;
	elseif($form_state['clicked_button']['#value'] == $form_state['values']['submittmpl']) 
		$uri = 'laporanbk1/' . $kodeuk . '/' . $jenis . '/'  . $tglawal . '/' . $tglakhir . '/'. $detil . '/tmpl/' . $tglcetak . '/' . $marginatas;
	else
		$uri = 'laporanbk1/' . $kodeuk . '/' . $jenis . '/'  . $tglawal . '/' . $tglakhir . '/'. $detil . '/xls/' . $tglcetak . '/' . $marginatas;
...
function getLaporanbk1($kodeuk, $jenis, $tglawal, $tglakhir, $detil, $tglcetak){
	...
	$rows[]=array(
		...
		array('data' => 'PEMERINTAH KABUPATEN JEPARA', 'width' => '775px','align'=>'center','style'=>'border:none;'),
		array('data' => 'BK-1|Panjar', 'width' => '50px','align'=>'center','style'=>'border:none;font-size:60%;'),
	);
	$rows[]=array(
	...
	$header[]=array(
		...
		array('data' => 'Panjar Bertambah', 'width' => '75px','align'=>'center','style'=>'border-right:1px solid black;border-left:1px solid black;border-bottom:1px solid black;font-size:75%;'),
		array('data' => 'Panjar Berkurang', 'width' => '75px','align'=>'center','style'=>'border-right:1px solid black;border-bottom:1px solid black;font-size:75%;'),
	);
	...
	$query->fields('b', array('bendid', 'spjno', 'noref', 'jenis', 'tanggal', 'total', 'keperluan', 'penerimanama', 'kodesuk'));
...
function getLaporanbk1_tmpl($kodeuk, $jenis, $tglawal, $tglakhir, $detil, $tglcetak){
	...
	$header[]=array(
		...
		array('data' => 'PANJAR', 'colspan'=>2,'width' => '150px','align'=>'center','style'=>'border-top:1px solid black;border-bottom:1px solid black;border-right:1px solid black;font-size:75%;'),
	);
	...
	$rows[]=array(
		...
		array('data' => apbd_fn($total_kas_lalu + $total_kas_ini + $total_panjar_lalu + $total_panjar_ini), 'colspan'=>2,'width' => '150px','align'=>'right','style'=>'border-right:1px solid black;border-bottom:1px solid black;border-top:1px solid black;font-size:75%;'),

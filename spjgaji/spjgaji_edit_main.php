<?php
function spjgaji_edit_main($arg=NULL, $nama=NULL) {
	...
	if(arg(3)=='pdf'){		
		...
		//$output .= theme('table', array('header' => $header, 'rows' => $rows ));
		//$output .= theme('pager');
		$output_form = drupal_get_form('spjgaji_edit_main_form');
		return drupal_render($output_form);// . $output;
	}		
	
}

function spjgaji_edit_main_form($form, &$form_state) {
	...
	$query->fields('d', array('bendid','keperluan', 'kodeuk', 'noref', 'spjno', 'tanggal'));
	...
	$form['formpajak'] = array (
		...
		foreach ($results as $data) {
			...
			$form['formpajak']['tablepajak']['e_kodepajak' . $i]= array(
					'#type' => 'value',
					'#value' => $kode,
			); 
		...
		if ($i==0) {
			...
			$query->fields('p', array('kodepajak', 'uraian'));
			$query->condition('p.kodepajak', '01', '=');
			$results = $query->execute();
			foreach ($results as $data) {
...
function spjgaji_edit_main_form_validate($form, &$form_state) {
	$spjno = $form_state['values']['spjno'];
	if (($spjno=='') or ($spjno=='BARU')) form_set_error('spjno', 'Nomor SPJ harap diisi dengan benar');
		
}
	
function spjgaji_edit_main_form_submit($form, &$form_state) {
	...
	for($n=1; $n<=$jumlahrekpajak; $n++){
		$e_kodepajak = $form_state['values']['e_kodepajak' . $n];

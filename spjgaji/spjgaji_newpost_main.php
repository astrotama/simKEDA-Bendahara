<?php
function spjgaji_newpost_main($arg=NULL, $nama=NULL) {
	//drupal_add_css('files/css/textfield.css');
	
	$dokid = arg(2);	
	$output_form = drupal_get_form('spjgaji_newpost_main_form');
	return drupal_render($output_form);// . $output;
	
}

function spjgaji_newpost_main_form($form, &$form_state) {
	...
	$query->fields('d', array('dokid', 'bulan', 'keperluan', 'kodeuk', 'sp2dno', 'sp2dtgl', 'jumlah', 'kodekeg', 'spjsudah'));
	...
	$form['formpajak'] = array (
		...
		$query->condition('dp.kodepotongan', '05', '=');
...
function spjgaji_newpost_main_form_validate($form, &$form_state) {
	$spjno = $form_state['values']['spjno'];
	if (($spjno=='') or ($spjno=='BARU')) form_set_error('spjno', 'Nomor SPJ harap diisi dengan benar');
		
}
	
function spjgaji_newpost_main_form_submit($form, &$form_state) {

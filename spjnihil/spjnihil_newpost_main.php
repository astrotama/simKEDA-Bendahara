<?php
function spjnihil_newpost_main($arg=NULL, $nama=NULL) {
	//drupal_add_css('files/css/textfield.css');
	
	$output_form = drupal_get_form('spjnihil_newpost_main_form');
	return drupal_render($output_form);// . $output;
	
}

function spjnihil_newpost_main_form($form, &$form_state) {
	...
	if (strpos($referer, 'arsip')>0)
		$_SESSION["spjnihillastpage"] = $referer;
	else
		$referer = $_SESSION["spjnihillastpage"];*/
	
	$kodekeg = arg(2);
	$jenis = arg(3);
	if ($jenis=='') $jenis = 'tu-nihil';
	...
	$opt_jenis['tu-nihil'] = 'TU NIHIL';
	$opt_jenis['gu-nihil'] = 'GU NIHIL';
	$form['jenis'] = array(
		'#type' => 'select',
		'#title' =>  t('Jenis SPJ'),
		'#options' => $opt_jenis,
	...
	$form['formdokumen']['tablerekening']= array(
		'#prefix' => '<div class="table-responsive"><table class="table"><tr><th width="10px">NO</th><th width="90px">KODE</th><th>URAIAN</th><th width="150px">ANGGARAN</th><th width="150px">JUMLAH</th><th width="200px">KETERANGAN</th></tr>',
		 '#suffix' => '</table></div>',
	);	
	$i = 0;
	$query = db_query('SELECT d.kodero,ro.uraian,d.jumlah FROM `anggperkeg` as d inner join rincianobyek as ro on d.kodero=ro.kodero WHERE d.jumlah>0 and kodekeg=:kodekeg', array(':kodekeg'=>$kodekeg));
	//$results = $query->execute();
...
function spjnihil_newpost_main_form_validate($form, &$form_state) {
...
function spjnihil_newpost_main_form_submit($form, &$form_state) {
